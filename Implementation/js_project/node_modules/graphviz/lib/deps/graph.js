...
Graph.prototype.addEdge = function(nodeOne, nodeTwo, attrs) {
  ...
  if( typeof(nodeOne) == 'string' ) {
    var nodeOneSelector = nodeOne.match(/\:.*/);
    var nodeOneName = nodeOne.replace(/\:.*/,'');
    _nodeOne = this.nodes.items[nodeOneName];
    if( _nodeOne == null) {
      console.log(nodeOneName);
      _nodeOne = this.addNode( nodeOneName );
    }
  }
  if( typeof(nodeTwo) == 'string' ) {
    var nodeTwoSelector = nodeTwo.match(/\:.*/);
    var nodeTwoName = nodeTwo.replace(/\:.*/,'');
    _nodeTwo = this.nodes.items[nodeTwoName];
    if( _nodeTwo == null) {
      console.log(nodeTwoName);
      _nodeTwo = this.addNode( nodeTwoName );
    }
  }
  
  var edge = new Edge(this, _nodeOne, _nodeTwo, nodeOneSelector, nodeTwoSelector);
  if( attrs ) {
    for( k in attrs ) {
      edge.set( k, attrs[k] );
    }
